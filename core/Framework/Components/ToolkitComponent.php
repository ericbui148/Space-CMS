 ...
 */
    ...
    {
    	$fields = Registry::getInstance()->get('fields');
 ...
 */
	public static function getCurrencySign($currency, $html=TRUE)
	{
		...
		return $html ? '<span class="form-field-icon-text-small">' . $currency . '</span>' : $currency;
	}
/**
...
{
	$text = ToolkitComponent::field($key);
	if ($return)
	{
		if (!$escape)
		{
		...
		if ($handle = opendir($dir))
		{
			...
			while (false !== ($file = readdir($handle)))
			{
				...
				if (!is_dir($dir . $sep . $file))
				{
					...
					ToolkitComponent::readDir($data, $dir . $sep . $file);
				}
			}
			closedir($handle);
		}
 ...
 */
	...
	/**
	 * Get Full class name for namespace
	 * @param string $classname
	 * @return string|boolean
	 */
	public static function getFullControllerClassName($classname)
	{
		...
		if (!empty($plugins)) {
			...
			foreach ($plugins as $plugin) {
				$fullClassName = "App\\Plugins\\$plugin\\Controllers\\".$classname;
				if (class_exists($fullClassName)) {
					return $fullClassName;
				}
			}			
	 ...
	 * @return string|boolean
	 */
	public static function getFullModelClassName($classname)
	{
		...
		if (!empty($plugins)) {
			$plugins = array_keys($plugins);
			foreach ($plugins as $plugin) {
				$fullClassName = "App\\Plugins\\$plugin\\Models\\".$classname;
				if (class_exists($fullClassName)) {
					return $fullClassName;
				}
			}			
