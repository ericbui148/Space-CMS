 ...
 * //...
 * $EmailComponent->detach('2013/09/3d5d32c134911623e025fbdf7edb1edf.pdf'); //Detach 1st attachmend
 * $EmailComponent->detach(NULL, 1); //Detach 2nd attachmend (indexes starts from 0)
 * </code>
 *
 * @param string $filename
 * @param int $index
			...
			case 'smtp':
				...
				try {
					...
					foreach($emails as $email) {
						$mail->AddAddress($email, $this->toName);
					}
 ...
 * <code>
 * $EmailComponent = new EmailComponent();
 * $EmailComponent->addPart('<html><head><title></title></head><body><strong>Some html</strong> text.<p>New paragraph goes here</p></body></html>', 'text/html');
 * //or
 * $EmailComponent->setContentType('text/html')->addPart('Some simple plain text', 'text/plain');
 * </code>
 *
