 ...
 *
 */
class MultibyteComponent extends Component
{
/**
 ...
 */
	public static function convert_encoding($str, $use_enc = null, $from_enc = null)
	{
		...
		if ( self::$use_library == self::USE_MBSTRING ) {
			if ( $from_enc == null ) {
				$from_enc = MultibyteComponent::detect_encoding($str);
			}
 ...
 */
	public static function substr($str, $begin, $len = null, $use_enc = null)
	{
		...
		if ( self::$use_library == self::USE_MBSTRING ) {
			if ( !isset($len) ) {
				$len = MultibyteComponent::strlen($str) - $begin;
			}
 ...
 */
	...
	{
		if (!MultibyteComponent::checkMultibyte($string) && strlen($string) < 75) {
			return $string;
		}
 ...
 */
	...
	{
		$check = MultibyteComponent::strtoupper($haystack);
		$check = MultibyteComponent::utf8($check);
		$found = false;

		$haystack = MultibyteComponent::utf8($haystack);
		$haystackCount = count($haystack);

		$matches = array_count_values($check);

		$needle = MultibyteComponent::strtoupper($needle);
		$needle = MultibyteComponent::utf8($needle);
		...
		if ($found && $part && !empty($parts)) {
			return MultibyteComponent::ascii($parts);
		} elseif ($found && !empty($haystack)) {
			return MultibyteComponent::ascii($haystack);
		}
		return false;
	}
/**
 ...
 */
	...
	{
		$check = MultibyteComponent::utf8($haystack);
		$found = false;
 ...
 */
	public static function stristr($haystack, $needle, $part = false)
	{
		...
		if (($php && $part) || MultibyteComponent::checkMultibyte($haystack)) {
			$check = MultibyteComponent::strtoupper($haystack);
			$check = MultibyteComponent::utf8($check);
			$found = false;
