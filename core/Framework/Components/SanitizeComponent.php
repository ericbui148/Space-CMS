	...
	public static function clean($data, $options = array())
	{
		...
		if (is_array($data)) {
			foreach ($data as $key => $val) {
				$data[$key] = SanitizeComponent::clean($val, $options);
			}
			return $data;
		}
		...
		if ($options['encode']) {
			$data = SanitizeComponent::html($data, array('remove' => $options['remove_html']));
		}
		if ($options['dollar']) {
 ...
 */
	...
	{
		return SanitizeComponent::stripScripts(
			SanitizeComponent::stripImages(
				SanitizeComponent::stripWhitespace($str)
			)
		);
	}
/**
 ...
 *
 * Ex.`$clean = SanitizeComponent::stripTags($dirty, 'b', 'p', 'div');`
 *
