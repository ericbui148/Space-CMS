...
class NewProductsWidget extends Widget
{
	...
	public function run($renderString = false) {
	    $widget = WidgetModel::factory()
    	    ...
    	    ->where('t1.type', WidgetModel::WIDGET_TYPE_NEW_PRODUCTS)
    	    ->findAll()
    	    ->limit(1)
    	    ->first();
	...
	{
	    $itemSortType = ItemSortModel::TYPE_WIDGET_PRODUCTS;
	    ...
	    ->join ('ItemSort', "t4.foreign_id = t1.id AND t4.foreign_type_id = $foreignTypeId AND t4.type = $itemSortType", 'left')
	...
	{
	    $ProductModel = ProductModel::factory()->select('id')->where ( 'status', ProductModel::STATUS_ACTIVE )->orderBy ( '`id` DESC' );
