...
(function ($, undefined) {
	...
	Multilang.prototype = {
		_attachMultilang: function (target, settings) {
			...
			}).addClass("form-langbar-tip selector-langbar-tip").appendTo($target);
			...
			for (i in inst.settings.langs) {
				if (inst.settings.langs.hasOwnProperty(i)) {
					...
					}).addClass("form-langbar-item").append($abbr).appendTo($target);
				}
			}
			$target.addClass("form-langbar");
			$target.find(".form-langbar-item[data-index='" + $(".multilang-wrap[style!='display: none']").data("index") + "']").addClass("form-langbar-item-active");

			$target.on("click", ".form-langbar-item", function (e) {
				...
				if (inst.settings.select !== null) {
					var $icon = $(".form-langbar-item-active", this).eq(0);
					inst.settings.select.call(target, e, {
						index: $icon.data("index"),
						abbr: $icon.data("abbr")
					});
				}
			}).on("click", ".selector-langbar-tip", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				return false;
		...
		_selectMultilang: function (target, index) {
			...
			var $icon = $(".form-langbar-item[data-index='" + index + "']");
			if ($icon.length > 0) {
				$icon
					.addClass("form-langbar-item-active")
					.siblings(".form-langbar-item").removeClass("form-langbar-item-active");
				
				var $wrapper = $(":input[name^='i18n[" + index + "]']").closest(".multilang-wrap");
				$(".multilang-wrap").hide();
				$wrapper.show();
