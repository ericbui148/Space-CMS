...
(function ($, undefined) {
	...
	Datagrid.formatDate = function (date, format) {
	    ...
	    if (Grid !== undefined && Grid.monthNames !== undefined) {
	    	_monthName = Grid.monthNames;
	    }
	    if (Grid !== undefined && Grid.dayNames !== undefined) {
	    	_dayName = Grid.dayNames;
	    }
	...
	Datagrid.prototype = {
		_attachDatagrid: function (target, settings) {
			...
			$target.addClass("grid").on("mouseenter.dg", ".table tbody tr", function () {
				$(this).addClass("table-row-hover");
			}).on("mouseleave.dg", ".table tbody tr", function () {
				$(this).removeClass("table-row-hover");
			}).on("change.dg", ".table-select-row", function () {
				var $this = $(this);
				if ($this.is(":checked")) {
			...
			}).on("change.dg", ".table-toggle-rows", function () {
				var $this = $(this),
					$grid = $this.closest(".grid");
				if ($this.is(":checked")) {
					$grid.find(".table-select-row").prop("checked", true).closest("tr").addClass("table-row-active");
				} else {
					$grid.find(".table-select-row").prop("checked", false).closest("tr").removeClass("table-row-active");
				}
			}).on("click.dg", ".table-sort-up, .table-sort-down", function (e) {
				...
				$.extend(inst.settings.cache, {
					...
					direction: $this.hasClass("table-sort-up") ? "ASC" : "DESC"
				});
				self._loadDatagrid.call(self, target, inst.settings.dataUrl, $this.data("column"), $this.hasClass("table-sort-up") ? "ASC" : "DESC", inst.settings.content.page, inst.settings.content.rowCount);
				return FALSE;
			}).on("click.dg", ".table-icon-delete", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				var $dialog = $("#dialog-delete-" + inst.uid),
					$this = $(this);
				if ($dialog.length > 0 && $.fn.dialog !== undefined) {
					$dialog.data("deleteurl", $this.attr("href")).data("row", $this.closest("tr")).dialog("open");
				}
				return FALSE;
			}).on("click.dg", ".table-icon-menu", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				var diff, lf,
				...
				return FALSE;
			}).on("click.dg", ".paginator-list-paginate", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				...
				return FALSE;
			}).on("click.dg", ".paginator-list-prev, .paginator-list-next", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				var $this = $(this),
					p = $this.hasClass("paginator-list-prev") ? inst.settings.content.page - 1 : inst.settings.content.page + 1;
				$.extend(inst.settings.cache, {
					page: p
				});
				self._loadDatagrid.call(self, target, inst.settings.dataUrl, inst.settings.content.column, inst.settings.content.direction, p, inst.settings.content.rowCount);
				return FALSE;
			}).on("click.dg", ".paginator-button-actions, .paginator-row-count", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				var diff, lf,
					$this = $(this),
					$list = $this.siblings(".menu-list-wrap").eq(0);
				...
				$(".menu-list-wrap").not($list).hide();
				return FALSE;
			}).on("click.dg", ".selector-row-count", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				...
				return FALSE;
			}).on("keypress.dg", ".selector-goto", function (e) {
			...
			}).on("click.dg", ".table-cell-editable", function (e) {
				var $this = $(this);
				$(".table-cell-label", $this).hide();
				$(".form-field", $this).show().focus();
			}).on("keypress.dg", ".table-cell-editable > .selector-editable", function (e) {
				switch (e.keyCode ? e.keyCode : e.which) {
					case 13: //Enter
						$(this).trigger("save");
						break;
				}
			}).on("keyup.dg", ".table-cell-editable > .selector-editable", function (e) {
				switch (e.keyCode ? e.keyCode : e.which) {
					case 27: //Escape
						$(this).hide().siblings(".table-cell-label").show();
						break;
				}
			}).on("save.dg", ".selector-editable", function (e) {
				var $this = $(this),
			...
			}).on("click.dg", ".paginator-action", function (e) {
				var i, $form,
					$this = $(this),
					$data = $this.data("object"),
					records = $(".table-select-row", $target).serializeArray(),
					iCnt = records.length;
				...
				if (iCnt > 0) {
					if ($data.confirmation !== undefined && $.fn.dialog !== undefined) {
						...
						$("#dialog-action-" + inst.uid).data("object", $data).dialog("open");
						...
						$.post($this.attr("href"), $(".table-select-row", $target).serialize()).done(function (data) {
							$this.closest(".menu-list-wrap").hide();
							if ($data.render) {
					...
					var $dialog = $("#dialog-action-empty");
					if ($dialog.length > 0 && $.fn.dialog !== undefined) {
						$dialog.dialog("open");
					}
					$this.closest(".menu-list-wrap").hide();
			...
			$(document).on("click.dg", "*", function (e) {
				if ($target.is(":visible")) {
					...
					if (!$(e.target).closest(".table-cell-editable:visible").length) {
						if ($(e.target).closest(".ui-datepicker").length > 0 || $(e.target).closest(".ui-datepicker-header").length > 0) {
							e.stopPropagation();
							return;
						}
						if ($(".selector-editable:visible").length > 0) {
							e.stopPropagation();
							$(".selector-editable:visible").trigger("save");
						}
					}
				}
			});
		...
		_renderDatagrid: function (target) {
			...
			for (i = 0, iCnt = inst.settings.columns.length; i < iCnt; i++) {
				...
				if (inst.settings.columns[i].sortable && inst.settings.content.data && inst.settings.content.data.length > 0) {
					$("<div>").addClass("table-sort-label").html(inst.settings.columns[i].text).appendTo($th);
					upClass = "";
					downClass = "";
					if (inst.settings.content.column == inst.settings.fields[i]) {
						if (inst.settings.content.direction == "ASC") {
							upClass = "table-sort-up-active";
						} else {
							downClass = "table-sort-down-active";
						}
					}
					$("<div>").addClass("table-sort")
						.append($("<a>", {"href":"#"}).data("column", inst.settings.fields[i]).addClass("table-sort-up").addClass(upClass))
						.append($("<a>", {"href":"#"}).data("column", inst.settings.fields[i]).addClass("table-sort-down").addClass(downClass))
						.appendTo($th);					
				} else {
			...
			if (inst.settings.content.data) {
				for (i = 0, iCnt = inst.settings.content.data.length; i < iCnt; i++) {
					...
					$tr = $("<tr>").addClass(i % 2 === 0 ? 'table-row-odd' : 'table-row-even').data("object", inst.settings.content.data[i])
						.attr("data-id", "id_" + inst.settings.content.data[i].id);
					if (inst.settings.sortable) {
						$tr.addClass("table-row-sortable");
					}
					if (inst.settings.select !== FALSE) {
						$td = $("<td>");
					...
					for (j = 0, jCnt = inst.settings.fields.length; j < jCnt; j++) {
						...
						$span = $("<span>").addClass("table-cell-label");
						if (inst.settings.columns[j].applyClass !== undefined) {
							$span.addClass("status").addClass(inst.settings.columns[j].applyClass + "-" + inst.settings.content.data[i][inst.settings.fields[j]]);
						}
						...
						if (inst.settings.columns[j].editable) {
							$td.addClass("table-cell-editable");
							switch (inst.settings.columns[j].type) {
								case "text":
								case "spinner":
								case "date":
									...
									}).hide().addClass("form-field form-text selector-editable");
								...
								case "select":
									$input = $("<select>", {
										"data-name": inst.settings.fields[j]
									}).hide().addClass("form-field form-select selector-editable");
									if (inst.settings.columns[j].saveUrl !== undefined) {
										$input.attr("data-saveurl", inst.settings.columns[j].saveUrl.replace(inst.settings.regex.params, function () {
											return inst.settings.content.data[i][arguments[1]];
										}));
					...
					if (bCnt > 0) {
						...
						for (j = 0; j < bCnt; j++) {
							...
							}).data("id", inst.settings.content.data[i]).addClass("table-icon-" + inst.settings.buttons[j].type);
							...
							if (inst.settings.buttons[j].type == "menu" && inst.settings.buttons[j].items !== undefined) {
								$a.removeClass("table-icon-menu").addClass("table-icon-menu table-button").html(inst.settings.buttons[j].text).append($("<span>").addClass("button-arrow-down"));
								$span = $("<span>").hide().addClass("menu-list-wrap");
								$("<span>").addClass("menu-list-arrow").appendTo($span);
								$ul = $("<ul>").addClass("menu-list");
			...
			if (inst.settings.paginator) {
				...
				if (inst.settings.paginator.actions && inst.settings.paginator.actions.length > 0) {
					$a = $("<a>", {
						"href": "#"
					}).addClass("button paginator-button-actions").html(self.messages.choose_action);
					$("<span>").addClass("button-arrow-down").appendTo($a);
					$a.appendTo($div);
					...
					if (kCnt > 0) {
						...
						buttons[self.messages.btn_ok] = function () {
							...
							$.post($data.url, $(".table-select-row", $target).serialize()).done(function (data) {
								if ($data.render) {
									self._loadDatagrid.call(self, target, inst.settings.dataUrl);
								}
								$this.dialog("close");
				...
				if (inst.settings.paginator.paginate) {
					...
					for (i = ii; i <= ic; i++) {
						$a = $("<a>", {
							...
							"data-page": i
						}).addClass("paginator-list-paginate").html(i);
						if (i == inst.settings.content.page) {
							$a.addClass("paginator-list-active");
						}
						$a.appendTo($ul);
					}
					if (inst.settings.content.pages > 1 && inst.settings.content.page != inst.settings.content.pages) {
						...
						}).addClass("paginator-list-next").html(self.messages.next).appendTo($ul);
					}
					$ul.appendTo($div);
				}
				
				$span = $("<span>").addClass("paginator-total");
				...
				if (inst.settings.paginator.rowCount) {
					$("<a>", {
						"href": "#"
					}).addClass("button paginator-row-count").html(inst.settings.content.rowCount).appendTo($span);
					
					$tmp = $("<span>").hide().addClass("menu-list-wrap");
					$("<span>").addClass("menu-list-arrow").appendTo($tmp);
					$ul = $("<ul>").addClass("menu-list");
					$("<li>").append($("<span>", {
						css: {"fontSize": "11px"}
					...
					for (i = 10; i < 60; i += 10) {
						$li = $("<li>").append( $("<a>", {
							"href": "#",
							"data-rowcount": i
						}).addClass("selector-row-count").html(i) ).appendTo($ul);
					}
					$ul.appendTo($tmp);
					$tmp.appendTo($span);
				}
		...
		bindSortable: function (target) {
			...
			$target.find(".table tbody").sortable({
				cursor: "move",
				helper: fixHelper,
				placeholder: "table-row-highlight",
		...
		_destroyDatagrid: function (target) {
			...
			$(target).removeClass("grid").off(".dg").html("");
			$(document).off(".dg");
