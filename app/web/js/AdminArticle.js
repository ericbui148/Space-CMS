...
(function ($, undefined) {
	$(function () {
		"use strict";
		var $frmCreateArticle = $("#frmCreateArticle"),
			$frmUpdateArticle = $("#frmUpdateArticle"),
		...
		if (chosen) {
			if($frmCreateArticle.length > 0 || $frmUpdateArticle.length > 0)
			{
				$("#tag_id").chosen({
				    create_option: true,
		...
		if ($frmCreateArticle.length > 0) 
		{
			...
			if(myLabel.locale_array.length > 0)
			{
				...
				{
					var element = $("#i18n_article_name_" + locale_array[i]),
		...
		function formatImage (path, obj) {
			...
			return ['<a href="index.php?controller=AdminArticle&action=Update&id=', obj.id, '"><img width="66px" src="', src, '" alt="" class="s-Img" /></a>'].join('');
		}

		if ($("#grid").length > 0 && datagrid) {
			var gridOpts = {
				buttons: [{type: "edit", url: "index.php?controller=AdminArticle&action=Update&id={:id}", title: myLabel.edit},
				          {type: "delete", url: "index.php?controller=AdminArticle&action=DeleteArticle&id={:id}", title: myLabel.delete}],
				columns: [
					      {text: myLabel.image, type: "text", sortable: true, editable: false, renderer: formatImage},
						  {text: myLabel.article, type: "text", sortable: true, editable: true, width: 200, editableWidth: 280},
				...
				dataUrl: "index.php?controller=AdminArticle&action=GetArticle",
				dataType: "json",
				fields: ['avatar_file', 'article_name', 'template', 'category', 'modified', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "index.php?controller=AdminArticle&action=DeleteArticleBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "index.php?controller=AdminArticle&action=SaveArticle&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
			...
			{
				gridOpts.buttons = [{type: "edit", url: "index.php?controller=AdminArticle&action=Update&id={:id}", title: myLabel.edit},
			   				          {type: "delete", url: "index.php?controller=AdminArticle&action=DeleteArticle&id={:id}", title: myLabel.delete},
							          {type: "menu", url: "#", text: myLabel.more, items:[
							              {text: myLabel.duplicate, url: "index.php?controller=AdminArticle&action=Create&id={:id}"},
							              {text: myLabel.install, url: "index.php?controller=AdminArticle&action=Install&id={:id}"},
							              {text: myLabel.preview, url: "{:url}", target: '_blank'}
							          ]}];
				if(Grid.isArticleAllowed == false)
				{
					gridOpts.buttons = [{type: "edit", url: "index.php?controller=AdminArticle&action=Update&id={:id}", title: myLabel.edit},
				   				          {type: "delete", url: "index.php?controller=AdminArticle&action=DeleteArticle&id={:id}", title: myLabel.delete},
								          {type: "menu", url: "#", text: myLabel.more, items:[
		...
		if ($("#history_grid").length > 0 && datagrid) {
			...
			var gridOpts = {
				buttons: [{type: "view", url: "index.php?controller=AdminArticle&action=View&id={:id}&article_id={:article_id}", title: myLabel.view},
				          {type: "edit", url: "index.php?controller=AdminArticle&action=Update&id={:article_id}", title: myLabel.edit},
				          {type: "delete", url: "index.php?controller=AdminArticle&action=DeleteHistory&id={:id}", title: myLabel.delete}
				         ],
				columns: [{text: myLabel.article, type: "text", sortable: true, editable: false, width: 220},
				...
				dataUrl: "index.php?controller=AdminArticle&action=GetHistory" + Grid.queryString,
				dataType: "json",
				fields: ['article_name', 'modified', 'name', 'ip'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "index.php?controller=AdminArticle&action=DeleteHistoryBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "index.php?controller=AdminArticle&action=SaveHistory&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
			...
			{
				gridOpts.buttons = [
				                    {type: "view", url: "index.php?controller=AdminArticle&action=View&id={:id}&article_id={:article_id}", title: myLabel.view},
				                    {type: "edit", url: "index.php?controller=AdminArticle&action=Update&id={:article_id}", title: myLabel.edit},
							        {type: "delete", url: "index.php?controller=AdminArticle&action=DeleteHistory&id={:id}", title: myLabel.delete}
		        ];
				gridOpts.columns = [
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "index.php?controller=AdminArticle&action=GetArticle", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.extend(cache, obj);
			$grid.datagrid("option", "cache", cache);
			$grid.datagrid("load", "index.php?controller=AdminArticle&action=GetArticle", "id", "DESC", content.page, content.rowCount);
			return false;
			...
			$grid.datagrid("option", "cache", cache);
			$grid.datagrid("load", "index.php?controller=AdminArticle&action=GetHistory", "modified", "DESC", content.page, content.rowCount);
			return false;
		}).on("change", "#article_id", function (e) {
			if($(this).val() == '')
			{
				window.location.href = "index.php?controller=AdminArticle&action=History";
			}else{
				window.location.href = "index.php?controller=AdminArticle&action=History&article_id=" + $(this).val();
			}
			...
			$dialogView.dialog({
				...
				open: function (event, ui) {
					...
					$('<a />', {
		                ...
		                href: 'index.php?controller=AdminArticle&action=PreviewHistory&id=' + $dialogView.data('id'),
		                target: '_blank'
		            })
		...
		function uploadFile(inp, editor) {
		  ...
		  $.ajax({
		    url: 'index.php?controller=Gallery&action=UploadArticleImage',
		    type: 'POST',
		    data: data,
