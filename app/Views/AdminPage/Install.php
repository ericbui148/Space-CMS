...
if (isset($tpl['status']))
{
	...
	<div class="ui-tabs ui-widget ui-widget-content ui-corner-all b10">
		<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
			<li class="ui-state-default ui-corner-top"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminPages&amp;action=Index"><?php __('menuPages'); ?></a></li>
			<?php
			if ($controller->isAdmin() || ($controller->isEditor() && $controller->isPageAllowed()))
			{ 
				?>
				<li class="ui-state-default ui-corner-top"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminPages&amp;action=Create"><?php __('lblAddPage'); ?></a></li>
				<?php
			}
			if($controller->isAdmin())
			{ 
				?>
				<li class="ui-state-default ui-corner-top"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminPages&amp;action=History"><?php __('lblChanges'); ?></a></li>
				<?php
			} 
			?>
			<li class="ui-state-default ui-corner-top ui-tabs-active ui-state-active"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminPages&amp;action=Install&amp;id=<?php echo $tpl['arr']['id'];?>"><?php __('lblInstall'); ?></a></li>
		</ul>
	</div>
	
	<?php 
	UtilComponent::printNotice(__('infoInstallPageTitle', true, false), __('infoInstallPageDesc', true, false)); 
	if($tpl['arr']['status'] == 'T')
	{
	?>
	
	<form id="frmInstallPage" class="form form" autocomplete="off">
	
		<p><span class="inline_block"><label class="content"><?php __('lblInstallNote');?>: <b><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminPages&amp;action=Update&amp;id=<?php echo $tpl['arr']['id'];?>"><?php echo SanitizeComponent::html($tpl['arr']['page_name']);?></a></b></label></span></p>
...
&lt;script type="text/javascript" src="<?php echo INSTALL_FOLDER; ?>index.php?controller=Front&action=Load&id=<?php echo $tpl['arr']['id'];?>{LOCALE}{HIDE}"&gt;&lt;/script&gt;</textarea>
	...
	<?php
		...
		$note = __('lblInactivePageNote', true);
		$page_link = '<b><a href="'.$_SERVER['PHP_SELF'].'?controller=AdminPages&amp;action=Update&amp;id='.$tpl['arr']['id'].'">'.SanitizeComponent::html($tpl['arr']['page_name']).'</a></b>';
		$note = str_replace("{PAGE}", $page_link, $note);
