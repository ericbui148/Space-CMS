...
if (isset($tpl['status']))
{
	...
	<div class="ui-tabs ui-widget ui-widget-content ui-corner-all b10">
		<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
			...
			<li class="ui-state-default ui-corner-top ui-tabs-active ui-state-active"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminArticle&amp;action=Create"><?php __('lblAddArticleCategory'); ?></a></li>
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminArticleCategories&amp;action=Create" method="post" id="frmCreateCategory" class="form form" autocomplete="off" enctype="multipart/form-data">
		<input type="hidden" name="category_create" value="1" />
		<p>
			<label class="title">Avatar</label>
			<span class="left" id="box_avatar">
				<input type="file" name="avatar" id="avatar" class="form-field w350"/>
			</span>
		</p>
		<?php
		...
		?>
			...
			<label class="title"><?php __('lblShortDescription'); ?>:</label>
				<span class="inline_block">
					<textarea rows="5" cols="50" id="i18n_desc_<?php echo $v['id'];?>" name="i18n[<?php echo $v['id']; ?>][desc]" class="form-field w500<?php echo (int) $v['is_default'] === 0 ? NULL : ' required'; ?>" lang="<?php echo $v['id']; ?>" ></textarea>
					<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
