	...
	public function getNode($locale_id, $id=null)
	{
	    ...
	    if (count($arr) === 1)
	    {
	    	...
	    	$descendants = $this->reset()
	    		...
	    		->join('MultiLang', "t2.foreign_id = t1.id AND t2.model = 'Category' AND t2.locale = '$locale_id' AND t2.field = 'name'", 'left outer')
	    		->join('MultiLang', "t3.foreign_id = t1.id AND t3.model = 'Category' AND t3.locale = '$locale_id' AND t3.field = 'description'", 'left outer')
