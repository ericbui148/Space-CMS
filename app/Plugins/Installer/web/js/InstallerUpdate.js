...
(function ($, undefined) {
	$(function () {
		...
		function formatButton(str, obj) {
			return ['<input type="button" value="', myLabel.execute, '" class="button btn-execute" data-name="', obj.name, 
			        '" data-module="', obj.module,
			        '" data-path="', obj.path, '" />'].join("");
		}
		
		function formatView (str, obj) {
			return ['<a href="index.php?controller=Installer&action=SecureView&p=', str, '" class="install-view" target="_blank"></a>',
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [],
				columns: columns,
				dataUrl: "index.php?controller=Installer&action=SecureGetUpdate",
				dataType: "json",
				fields: fields,
				paginator: false,
				saveUrl: null,
		...
		if ($dialogExecuteAll.length > 0 && dialog) {
			$dialogExecuteAll.dialog({
				...
				buttons: {
					"Execute": function () {
						$.post("index.php?controller=Installer&action=SecureSetUpdate", $grid.find('input[name="record[]"], input[name="module[]"]').serialize()).done(function (data) {
							if (data.status == "OK") {
								...
								$grid.datagrid("load", "index.php?controller=Installer&action=SecureGetUpdate", "", "", content.page, content.rowCount);
		...
		if ($dialogExecute.length > 0 && dialog) {
			$dialogExecute.dialog({
				...
				buttons: {
					"Execute": function () {
						$.post("index.php?controller=Installer&action=SecureSetUpdate", {
